...
public class WatchingSessionService {
    ...
    @Caching(put = {
        @CachePut(cacheNames = "watching_sessions", key = "#model.content.id + ':' + #model.watcher.id")
    })
    public WatchingSessionModel create(WatchingSessionModel model) {
        return watchingSessionRepository.save(model);
    }

    @CacheEvict(cacheNames = "watching_sessions", key = "#model.content.id + ':' + #model.watcher.id")
