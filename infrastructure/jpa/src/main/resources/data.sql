...
VALUES
    ...
    (CAST('019439c0-0006-7000-8000-000000000006' AS UUID), DATEADD('DAY', -62, NOW()), NULL, DATEADD('DAY', -62, NOW()), 'MOVIE', '하나비 (1997)', '형사인 니시(기타노 다케시)와 호리베(렌 오스기)는 야쿠자 소탕 전문형사 콤비이자 친한 친구 사이. 그러던 어느 날, 니시는 딸을 잃고 아내가 시한부 판정까지 받게 된다. 잠복 근무 중 동료들의 호의로 니시가 아내의 병문안을 간 사이 불의의 습격을 받은 호리에는 불구가 되어 가족들에게 버림을 받는다. 게다가 호리에를 습격한 범인이 니시를 따르던 후배 경찰까지 죽이자 이에 분노한 니시는 그 자리에서 마지막 총알까지 다 퍼부으면서 범인을 죽인 후 경찰직을 그만둔다. 한편 아내의 치료비 때문에 큰 빚을 지게 된 니시는 경찰을 사칭하여 은행을 터는데...', 'http://localhost:8080/api/v1/files/display?path=contents/019439c0-0006-7000-8000-000000000006/f6a7b8c9-d0e1-4f2a-3b4c-5d6e7f8a9b0c_thumbnail.webp', 0.0, 0),
    (CAST('019439c0-0007-7000-8000-000000000007' AS UUID), DATEADD('DAY', -58, NOW()), NULL, DATEADD('DAY', -58, NOW()), 'MOVIE', '퐁네프의 연인들 (1991)', '파리 센느강의 아홉 번째 다리 퐁네프. 사랑을 잃고 거리를 방황하며 그림을 그리는 여자 미셸, 폐쇄된 퐁네프 다리 위에서 처음 만난 그녀가 삶의 전부인 남자 알렉스. 마치 내일이 없는 듯 열정적이고 치열하게 사랑한 두 사람. 한 때 서로가 전부였던 그들은 3년 뒤, 크리스마스에 퐁네프의 다리에서 재회하기로 하는데...', 'http://localhost:8080/api/v1/files/display?path=contents/019439c0-0007-7000-8000-000000000007/a7b8c9d0-e1f2-4a3b-4c5d-6e7f8a9b0c1d_thumbnail.jpg', 0.0, 0);
